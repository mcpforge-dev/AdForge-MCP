# Подключение hosted HolyMedia MCP в Codex
...
8. Сохранить сервер.
9. Проверить, что tools HolyMedia MCP появились в списке.
...
- в новом чате Codex видит HolyMedia MCP и отвечает на запрос `Какие рекламные аккаунты подключены?`;
