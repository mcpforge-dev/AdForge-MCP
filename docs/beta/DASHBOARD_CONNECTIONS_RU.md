...
## Добавление ещё одного кабинета

После первого подключения на карточке платформы появляется кнопка `Добавить кабинет`. Она повторно открывает OAuth-выбор, но не заменяет уже подключённые кабинеты: старые строки помечаются как `Уже подключён`, а пользователь выбирает только новые. При сохранении сервер объединяет новый выбор с текущим workspace-scoped списком и обновляет OAuth credentials, поэтому забытый четвёртый кабинет можно добавить без риска потерять первые три.
