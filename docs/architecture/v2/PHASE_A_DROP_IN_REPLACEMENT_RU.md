...
Самостоятельный OAuth клиента остаётся основным сценарием и использует сохранённый
callback `/oauth/meta/callback`. Ручная заявка является дополнительным support
flow. Специалисту не выдаётся членство `ADMIN` или `OWNER` в клиентском workspace:
для него существует отдельный отзываемый и ограниченный по времени grant
`support.connection_requests.manage`. Он разрешает только обработку заявки,
запуск привязанного к ней OAuth и выбор кабинета из ответа Meta. Выбранный аккаунт
добавляется к уже включённым, поэтому повторный выбор не отключает ранее
подключённые кабинеты. Все действия фиксируются в audit log.
